...
class MultiViewUniMatch(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        intrinsics = intrinsics.clone()    # 调整内参矩阵（从归一化坐标还原到像素坐标）
        intrinsics[:, :, 0] *= ori_w # fx *= width
        intrinsics[:, :, 1] *= ori_h # fy *= height
        ...
        features_list_mv = self.transformer(  # 多视图Transformer融合
            features_list,
        ...
        resize_h, resize_w = ori_h // 14 * 14, ori_w // 14 * 14 # 调整图像尺寸为14的倍数（满足ViT要求）
        concat = rearrange(images, "b v c h w -> (b v) c h w")
        ...
        for scale_idx in range(self.num_scales):
            ...
            if scale_idx > 0:
                ...
                depth = F.interpolate(   #[6, 1, 64, 112]
            ...
            if scale_idx == 0:
                ...
                linear_space = (
                    torch.linspace(0, 1, num_depth_candidates)  # 32
                    .type_as(features_list_cnn[0])
                    .view(1, num_depth_candidates, 1, 1)
                )  # [1, D, 1, 1]
            ...
            #         align_corners=True,
            #         )
            #     # results_dict.update({"pdf": pdf})
            #     results_dict.update({"pdf": match_prob})    #[B, 128, 32, 56]
