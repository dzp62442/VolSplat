...
class DecoderSplattingCUDA(Decoder[DecoderSplattingCUDACfg]):
    ...
    def forward(
        ...
        b, v, _, _ = extrinsics.shape  #b = 2, v = 8
        color = render_cuda(
            rearrange(extrinsics, "b v i j -> (b v) i j"),   #[2, 8, 4, 4]  -> [16, 4, 4] 
            rearrange(intrinsics, "b v i j -> (b v) i j"),   #[2, 8, 3, 3]  -> [16, 3, 3] 
            rearrange(near, "b v -> (b v)"), #[2, 8] -> [16]
            rearrange(far, "b v -> (b v)"),  #[2, 8] -> [16]
            image_shape,
            repeat(self.background_color, "c -> (b v) c", b=b, v=v),
            repeat(gaussians.means, "b g xyz -> (b v) g xyz", v=v),   #[2, 256000, 3] --> [16, 256000, 3]
