...
class ViewSamplerBoundedV2(ViewSampler[ViewSamplerBoundedV2Cfg]):
    ...
    def sample(
        ...
        elif self.cfg.context_gap_warm_up_steps > 0:   # Dynamic context gap control
        ...
        if self.stage != "test" and self.cfg.target_gap_warm_up_steps > 0:
            max_target_gap = self.schedule(   # Target view gap calculation
        ...
        index_context_left = torch.randint(   # Randomly select left boundary viewpoint
            low=0,
        ...
        index_context_right = index_context_left + context_gap   # Determine right boundary
        # Target viewpoint boundary calculation
        ...
        if self.stage == "test":
            ...
            else:  # sample without replacement - random sampling without replacement
        ...
        if total_num_views > 2:
            ...
            extra_views = []
            if self.cfg.extra_views_sampling_strategy == 'random':  # Simple random sampling
            ...
            elif self.cfg.extra_views_sampling_strategy == 'farthest_point':  # Farthest point sampling to ensure coverage
